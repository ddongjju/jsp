...
//		FileInputStream fis = new FileInputStream("D:\\A_TeachingMaterial\\6.JspSpring\\workspace\\spring\\src\\test\\resources\\kr\\or\\ddit\\upload\\moon.png");
		MockMultipartFile file = new MockMultipartFile("file", "moon.png", "image/png", is);
		mockMvc.perform(fileUpload("/fileupload/upload").
				file(file).param("userid", "brown")).andExpect(view().name("fileupload/fileupload")).andExpect(status().isOk());
	}
